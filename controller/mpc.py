...
class MPC(Controller):
 def __init__(self, global_path, robot_dynamics, Q):
     ...
     self.tau_b       = 20                            # maximum boundry for the wheels torque
 ...
 def setup(self, x0, ref_path):
  ...
  Args:
   x0:       current pose of the robot. It's an array of size N,
   ref_path: reference path as an array of size N*horizon_len
  ...
  for i in range(self.horizon_len):
   ...
   self.costs["input_stage"] += ca.mtimes(self.variables["u"][:, i].T, ca.mtimes(self.R, self.variables["u"][:, i]))
